...
7. The cron API fetches live bars, evaluates live-enabled strategies, applies risk guards, stores alerts, and optionally sends Telegram messages.
...
- `vercel.json`: Vercel cron schedule for `/api/cron/check-signals`.
...
- `src/app/api/cron/check-signals/route.ts`: live signal check endpoint used by cron and manual calls.
...
- `/api/cron/check-signals` fetches recent market bars, evaluates live-enabled strategies, runs Topstep-style risk guards, deduplicates alerts, stores them, and optionally sends Telegram messages.
